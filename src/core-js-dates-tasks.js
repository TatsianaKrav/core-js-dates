...
function getNextFriday(date) {
  ...
  let daysCount = 0;
  if (dayOfWeek < 5) daysCount = 5 - dayOfWeek;
  if (dayOfWeek >= 5) daysCount = 7 - dayOfWeek + 5;
...
function isDateInPeriod(date, period) {
  ...
  const perdiodDiff = (end - start) / (1000 * 60 * 60 * 24) + 1;
  const dateDiff = (formattedDate - start) / (1000 * 60 * 60 * 24);

  return dateDiff <= perdiodDiff && dateDiff >= 0;
}
...
function formatDate(date) {
  ...
  let secs = formattedDate.getSeconds();
  let dayTime = '';

  dayTime = hours >= 0 && hours < 12 ? 'AM' : 'PM';
  hours = hours >= 0 && hours <= 12 ? hours : hours - 12;
  secs = secs < 10 ? `0${secs}` : secs;
  mins = mins < 10 ? `0${mins}` : mins;

  return `${month}/${day}/${year}, ${hours}:${mins}:${secs} ${dayTime}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  const bothWeek = new Date(year, month - 1, daysPerMonth).getDay() !== 6;
  let isSunday = false;

  let daysToWeeks = 0;
  if (firstDayOfMonth !== 0) {
    daysToWeeks = 6 - firstDayOfMonth;
  } else {
    isSunday = true;
  }
  ...
  let restDays = 1 + daysToWeeks;

  while (restDays <= daysPerMonth) {
    totalCount += 1;
    restDays += 7;
  }

  totalCount = isSunday ? totalCount * 2 - 1 : totalCount * 2;
  totalCount = bothWeek ? totalCount : totalCount - 1;
...
function getWeekNumberByDate(date) {
  let dateUTC = new Date(date.getFullYear(), date.getMonth(), date.getDate());
  dateUTC = new Date(dateUTC.setTime(dateUTC.getTime() + 1));

  const firstJan = new Date(dateUTC.getFullYear(), 0, 1);
  const isMonday = firstJan.getDay() === 1;
  const day = firstJan.getDay() !== 0 ? 7 - firstJan.getDay() + 1 : 1;
  const countDate =
    firstJan.getDay() === 1
      ? new Date(dateUTC.getFullYear(), 0, 1)
      : new Date(dateUTC.getFullYear(), 0, 1 + day);

  const daysDiff = (dateUTC - countDate) / (24 * 60 * 60 * 1000);

  let result = Math.ceil(daysDiff / 7);
  result = isMonday ? result : result + 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const dateStartArr = period.start.split('-');
  const dateEndArr = period.end.split('-');
  let dateStart = new Date(
    dateStartArr[2],
    dateStartArr[1] - 1,
    dateStartArr[0]
  );
  const dateEnd = new Date(dateEndArr[2], dateEndArr[1] - 1, dateEndArr[0]);
  const arr = [];
